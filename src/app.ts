...
import { initMobileCarousels } from './mobile-carousel';
...
const media = (src: string, alt: string, className = '') => src.startsWith('[PLACEHOLDER:')
  ? `<div class="media-placeholder ${className}" role="img" aria-label="Photo coming soon"><span>Sample image</span><strong>${tokenText(src).replace(' PHOTO', '').replace(' IMAGE', '')}</strong><i aria-hidden="true"></i></div>`
  : `<figure class="brand-media ${className}"><img src="${src}" alt="${alt}" loading="lazy" decoding="async"><figcaption>Concept image</figcaption></figure>`;
...
app.innerHTML = `
  ...
  <header class="site-header">
    <a class="brand" href="#top" aria-label="Trisomy21 Pickle Court home"><img src="/assets/trisomy21-logo.png" alt="" width="74" height="68"><span class="sr-only">Trisomy21 Pickle Court</span></a>
  ...
  <main id="main">
    <section class="hero" id="top">
      ...
      <div class="hero-visual"><figure class="brand-media hero-image"><img src="/assets/trisomy21-hero.png" alt="Trisomy21 Pickle Court brand artwork over an illuminated night court" decoding="async"><figcaption>Official brand artwork</figcaption></figure><div class="court-line" aria-hidden="true"></div></div>
    </section>

    <section class="section" id="courts"><div class="section-heading"><p class="eyebrow">Play here</p><h2 id="courts-title">Two courts. One welcoming club.</h2><p>Choose your side, bring your favorite people, and settle in for a good match.</p></div>
      <div class="court-list collection-track" data-carousel data-carousel-type="Court" data-carousel-heading="courts-title">${courts.length ? courts.map((court, index) => `<article class="court-feature ${index % 2 ? 'reverse' : ''}" data-carousel-slide>${media(court.image, court.alt)}<div><p class="number">0${index + 1}</p><h3>${court.name}</h3><p>${court.summary}</p><ul>${court.features.map(feature => `<li>${feature}</li>`).join('')}</ul><p class="price">${samplePrice(court.rateDisplay)}</p><a class="text-link court-link" href="#schedule" data-court="${court.slug}">See this court’s schedule →</a></div></article>`).join('') : '<p class="collection-empty">No courts are listed right now.</p>'}</div>
    ...
    <section class="section canteen" id="canteen"><div class="canteen-intro"><div class="section-heading"><p class="eyebrow">Stay a little longer</p><h2 id="canteen-title">From match point<br>to meal time.</h2><p>Grab a quick bite, cool down with a drink, and catch up after the last rally. Everything is ordered right at the canteen.</p><p class="sample-note">Sample canteen hours: daily, 8:00 AM–9:00 PM</p></div>${media('/assets/canteen.png', 'Concept image of a courtside canteen serving a rice bowl, sandwich and iced calamansi')}</div><div class="menu-list"><ul class="menu-track collection-track" data-carousel data-carousel-type="Menu item" data-carousel-heading="canteen-title">${menu.length ? menu.map(item => `<li class="menu-slide" data-carousel-slide><article class="menu-item"><div><p class="category">${item.category}</p><h3>${item.name}</h3><p>${item.description}</p><span class="sample-tag">${item.availabilityLabel}</span></div><strong>${samplePrice(item.priceDisplay)}</strong></article></li>`).join('') : '<li class="collection-empty">The sample canteen menu is not available right now.</li>'}</ul><p class="counter-note"><strong>Order at the canteen</strong><br>Menu and availability shown here are sample content.</p></div></section>

    <section class="shop-section" id="shop"><div class="section"><div class="section-heading"><p class="eyebrow">Club essentials</p><h2 id="shop-title">Play with the right kit.</h2><p>Sample merchandise and practical court gear from Trisomy21. Inventory and pricing are not live.</p></div><div class="product-list collection-track" data-carousel data-carousel-type="Club essential" data-carousel-heading="shop-title">${products.length ? products.map(product => `<article class="product" data-carousel-slide>${media(product.image, `Concept image of ${product.name}`)}<div><p class="category">${product.category}</p><h3>${product.name}</h3><p>${product.description}</p><ul>${product.variants.map(v => `<li>${v}</li>`).join('')}</ul><p class="price">${samplePrice(product.priceDisplay)}</p><button class="text-button inquiry" data-product="${product.id}" type="button">${product.actionType === 'quote' ? 'Request a quote' : product.id === 'p1' ? 'Ask about this shirt' : 'Ask about pickleballs'} →</button></div></article>`).join('') : '<p class="collection-empty">No club essentials are listed right now.</p>'}</div></div></section>
  ...
  <footer><a class="brand footer-brand" href="#top" aria-label="Trisomy21 Pickle Court home"><img src="/assets/trisomy21-logo.png" alt="" width="96" height="88"><span class="sr-only">Trisomy21 Pickle Court</span></a><nav aria-label="Footer"><a href="#courts">Courts</a><a href="#schedule">Schedule</a><a href="#canteen">Canteen</a><a href="#shop">Shop</a></nav><p>Website preview · Details are sample content.<br>© 2026 Trisomy21 Pickle Court</p></footer>
