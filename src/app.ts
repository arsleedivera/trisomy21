...
app.innerHTML = `
  ...
  <main id="main">
    ...
    <section class="section" id="courts"><div class="section-heading"><p class="eyebrow">Play here</p><h2>Two courts. One welcoming club.</h2><p>Choose your side, bring your favorite people, and settle in for a good match.</p></div>
      <div class="court-list">${courts.map((court, index) => `<article class="court-feature ${index % 2 ? 'reverse' : ''}">${media(court.image, court.alt)}<div><p class="number">0${index + 1}</p><h3>${court.name}</h3><p>${court.summary}</p><ul>${court.features.map(feature => `<li>${feature}</li>`).join('')}</ul><p class="price">${samplePrice(court.rateDisplay)}</p><a class="text-link court-link" href="#schedule" data-court="${court.slug}">See this court’s schedule →</a></div></article>`).join('')}</div>
